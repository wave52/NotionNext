...
const MenuList = props => {
  ...
  let links = [
    { icon: 'fas fa-home', name: locale.NAV.INDEX, to: '/' || '/', show: true },
    {
      ...
      to: '/category',
      show: CONFIG_MATERY.MENU_CATEGORY
    },
    {
      icon: 'fas fa-tag',
      name: locale.COMMON.TAGS,
      to: '/tag',
      show: CONFIG_MATERY.MENU_TAG
    },
    {
      icon: 'fas fa-archive',
      ...
      to: '/archive',
      slot: archiveSlot,
      show: CONFIG_MATERY.MENU_ARCHIVE
    },
    {
      icon: 'fas fa-search',
      name: locale.NAV.SEARCH,
      to: '/search',
      show: CONFIG_MATERY.MENU_SEARCH
    }
  ]
  if (customNav) {
        ...
        if (link && link.show) {
          ...
          return (
            <Link
              ...
              href={link.to}
              className={
                'py-1.5 px-5 text-base justify-between hover:bg-amber-400 hover:text-white hover:shadow-lg cursor-pointer font-light flex flex-nowrap items-center ' +
